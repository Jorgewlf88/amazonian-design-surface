...
| Total Area Coverage | **300 % maximum** for reactive and pigment digital textile printing |
...
| Minimum tile dimensions | 2000x2000 px (about 16.9x16.9 cm at 300 DPI) |
...
1. **Repeat type declared.** `full-drop`, `half-drop` or `brick` with an explicit offset percentage,
   recorded in `meta.json`, field `repeat.type`.
...
3. **Bleed, do not clip.** No artwork terminates on the artboard edge. Elements extend past the
   boundary and wrap.
4. **Proof required.** A 3x3 tiled render named `*-repeat-proof.png` accompanies every submission.
...
6. **Scale honesty.** The physical repeat size in `meta.json` must equal the pixel dimensions divided
   by 300 DPI.
7. **Motif distribution.** Avoid unintended diagonal "trailing": check the 3x3 proof at 25 % zoom,
...
| `blnd` | 2-12 cm repeat. Reads near-solid at 2 m. | **6 flat colours maximum**, vector preferred |
